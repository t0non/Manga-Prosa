...
export default function QuickChoices({ title = "Tá com fome de quê hoje?" }: { title?: string }) {
  const choices = [
  ...
  return (
    <section className="py-12 bg-brand-straw relative z-20 overflow-hidden">
      ...
      <div className="absolute top-0 -left-10 md:-top-6 md:-left-10 w-32 h-44 md:w-40 md:h-56 opacity-100 blur-[1px] pointer-events-none z-30">
        <Image src={efeitoCafeImg} alt="" fill className="object-contain" />
      </div>

      {/* Garfo no lado direito embaixo — sutil */}
      <div className="absolute -bottom-16 -right-10 md:-bottom-32 md:-right-20 w-40 h-52 md:w-56 md:h-[320px] opacity-100 pointer-events-none -rotate-12 z-30">
